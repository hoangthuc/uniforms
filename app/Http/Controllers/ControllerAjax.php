...
class ControllerAjax extends Controller
{
    public function admin_ajax(Request $request)
    {
        ...
        if (isset($request['action']) && $request['action'] == 'update_media') {
            $resulf = [];
            DB::table('media')->where('id', $request['id'])->update([
                    'title'       => $request['title'],
        ...
        if (isset($request['action']) && $request['action'] == 'update_product_category') {
            ...
            if ($data['feature_thumbnail']) {
                $user_id = Auth::id();
                Options::update_option($user_id, 'product_categories_thumbnail_'.$data['id'],
                        $data['feature_thumbnail']);
            }
        ...
        if (isset($request['action']) && $request['action'] == 'update_account') {
            ...
            foreach ($data as $key => $value) {
                ...
                if ($value['type'] == 'main') {
                    $data_user[$value['name']] = $value['value'];
                }
            }
            ...
            if ($data_user) {
                User::update_user($user_id, $data_user);
            }
            $resulf['success'] = 'successfully';
            echo \GuzzleHttp\json_encode($resulf);
        ...
        if (isset($request['action']) && $request['action'] == 'save_product_attributes') {
            ...
            if (isset($all_attribute['optional'])) {
                Product::update_meta_product($product_id, 'product_variations',
                        \GuzzleHttp\json_encode($all_attribute['optional']));
            }
            if (isset($all_attribute['attributes'])) {
                Product::update_meta_product($product_id, 'all_attributes',
                        \GuzzleHttp\json_encode($all_attribute['attributes']));
                $attributes = [];
        ...
        if (isset($request['action']) && $request['action'] == 'filter_product_page') {
            ...
            $resulf['total'] = format_currency($filter_products['pagition']['total']).' results';
        ...
        if (isset($request['action']) && $request['action'] == 'add_to_order') {
            ...
            if ($order_id) {
                ...
                if (isset($data['order'])) {
                    Orders::update_meta_product_order($order_id, 'orders', json_encode($data['order']));
                }
        ...
        if (isset($request['action']) && $request['action'] == 'import_categories') {
            ...
            if ($xlsx = SimpleXLSX::parse($file)) {
                ...
                foreach ($data as $row) {
                    if ($row[0]) {
                        $parent_id = Product::check_categories($row[0]);
                    }
                    if (isset($parent_id)) {
        ...
        if (isset($request['action']) && $request['action'] == 'get_attribute_ajax_view') {
            ...
            $price_attribute = isset($request['product_id']) ? Product::get_meta_product($request['product_id'],
                    'price_attribute') : '';
            $price_attribute = ($price_attribute) ? (array)\GuzzleHttp\json_decode($price_attribute) : [];

            $default_attribute = isset($request['product_id']) ? Product::get_meta_product($request['product_id'],
                    'default_attribute') : '';
            $default_attribute = ($default_attribute) ? (array)\GuzzleHttp\json_decode($default_attribute) : [];

            $thumbnail_color = isset($request['product_id']) ? Product::get_meta_product($request['product_id'],
                    'thumbnail_color') : '';
            $thumbnail_color = ($thumbnail_color) ? (array)\GuzzleHttp\json_decode($thumbnail_color) : [];

            $thumbnail_attribute = isset($request['product_id']) ? Product::get_meta_product($request['product_id'],
                    'thumbnail_attribute') : '';
            $thumbnail_attribute = ($thumbnail_attribute) ? (array)\GuzzleHttp\json_decode($thumbnail_attribute) : [];

            return view('admin.products.template_attribute_ajax_view',
                    compact('detail_attribute', 'item', 'price_attribute', 'default_attribute', 'thumbnail_color',
                            'thumbnail_attribute'));
        }
        ...
        if (isset($request['action']) && $request['action'] == 'get_name_plate_ajax_view') {
            ...
            $img = '';
            return view('admin.products.template_name_plate_view',
                    compact('detail_attribute', 'plate', 'name_plate', 'img'));
        }
        ...
        if (isset($request['action']) && $request['action'] == 'update_address_order') {
            ...
            if ($_POST['field'] == 'shipping_address') {
                $shipping_address = $data;
            }
            if ($_POST['field'] == 'billing_address') {
                $billing_address = $data;
            }
            if (isset($data['Tracking'])) {
                $tracking_order = $data;
            }
            return view('admin.order.view-ajax-order',
                    compact('shipping_address', 'billing_address', 'tracking_order'));
        }
...
//            $price_attribute = Product::get_meta_product($productId,'price_attribute');
...
//        }
        if (isset($request['action']) && $request['action'] == 'admin_get_product_order_info') {
            ...
            $orderMeta = Orders::get_meta_product_order($orderId, 'products');
            $orderMeta = collect(json_decode($orderMeta));
            $productOrderMeta = $orderMeta->filter(function ($product) use ($productId) {
                return $product->product_id == $productId;
            })->first();
            $product = Product::get_product($request->get('content')['productId']);
            ...
            if (isset($get_reviews)) {
                foreach ($get_reviews as $item_review) {
            ...
            foreach ($productOrderMeta->attributes as $obj) {
                $productOrderAttribute[str_replace(' ','_',key($obj))] = $obj->{key($obj)};
            }
            $name_plate_array = [];
            $name_plate_order=[];
            foreach ($name_plates as $value) {
                ...
                $name_plate = \App\Product::product_name_line()[$plate];
                $line = 'line_1';
                if ($plate == '2_even' || $plate == '2_r_small') {
                    $line = 'line_2';
                }
                if ($plate == '3_even' || $plate == '3_r_small') {
                    $line = 'line_3';
                }
                $selected = false;
//                dump($color,$line,$productOrderMeta->key,'-color:'. strpos( $productOrderMeta->key,'_'.$color)
//                        ||
//                        strpos( $productOrderMeta->key,$color.'_')
//                        ||
//                        strpos( $productOrderMeta->key,'_'.$color.'_'));
                if(
                        (bool)((
                                strpos( $productOrderMeta->key,'_'.$color)
                                ||
                                strpos( $productOrderMeta->key,$color.'_')
                                ||
                                strpos( $productOrderMeta->key,'_'.$color.'_')
                        ...
                        &&
                        strpos( $productOrderMeta->key,$line))
                )
                {
                    $selected = true;
                    $name_plate_order = [
                            ...
                            'key'      => $value->key,
                            'plate'    => $value->plate,
                            'name'     => $name_plate.' '.$detail_attribute->name,
                            'selected' => $selected
                    ];
                }
                $name_plate_array[] = [
            ...
            $name_plates = $name_plate_array;
            $productAlterations = null;
            if (property_exists($productOrderMeta, 'alterations') && isset($productOrderMeta->alterations[0])) {
                $productAlterations = $productOrderMeta->alterations[0];
            }
            $html = view('admin.order.edit_product_order',
                    compact('product', 'category', 'price', 'shipping', 'sku', 'thumbnail_color'
                            , 'price_attribute', 'default_attribute', 'attributes', 'relation_product',
                            'variantions', 'name_plates', 'reviews', 'list_reviews', 'pagition',
                            'id_color', 'galleries', 'outstock', 'all_price', 'outstock_hemming',
                            'productAlterations','name_plate_order',
                            'productOrderMeta', 'productOrderAttribute'))
                    ->render();
        ...
        if (isset($request['action']) && $request['action'] == 'admin_update_product_order') {
            ...
            $products = Orders::get_meta_product_order($request->order_id, 'products');
            $resulf = ['html' => ''];
            if ($products) {
                $products = (json_decode($products));
                $array_product = [];
                foreach ($products as $key => $product) {
                    ...
                    {
                        $array_product[$key]=(object)$order;
                    }
                    else
                        $array_product[$key]=$product;

                }
                Orders::update_meta_product_order($request->order_id, 'products', json_encode($array_product));
                $updatedMeta = Orders::get_meta_product_order($request->order_id, 'products');
                $orderTotalMeta = Orders::get_meta_product_order($request->order_id, 'total');
                $updatedMeta = json_decode($updatedMeta);
                $orderTotalMeta = json_decode($orderTotalMeta);
                $subtotal=0;
                foreach($updatedMeta as $key => $value)
                {
                    $subtotal += $value->subtotal;
                }
                $total = $orderTotalMeta->shipping+$subtotal+(($subtotal*$orderTotalMeta->tax)/100);
                $updateOrderTotalMeta =[
                    'tax'=>$orderTotalMeta->tax,
                    'shipping'=>$orderTotalMeta->shipping,
                    'subtotal'=>$subtotal,
                    'total'=>$total,
                ];
                Orders::update_meta_product_order($request->order_id, 'total', json_encode((object)$updateOrderTotalMeta));
                Orders::updateOrder($request->order_id,['total'=>$total]);
                $resulf = display_product_in_order($array_product, $request->order_id);
            }
            return $resulf['html'];
        }
