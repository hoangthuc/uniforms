    ...
    public static function add_product_excel($data)
    {
        ...
        if($data['gallery']){
            ...
            foreach ($galleries as $image){
                ...
                $gallery[] = ['id'=> $id, 'link'=> \App\Media::get_url_media($id)];
            }
        ...
        if( isset($data['variations']) ){
            $product_variations = [];
            foreach ($data['variations'] as $variants){
            $Product_var = [];
            $Product_var['price'] = $variants['price'];
            $Product_var['description'] = $variants['description'];
            $Product_var['img'] = Media::create_media_from_path($data['path'].'/'.$variants['featured_image'],$variants['featured_image']);
            $Product_var['sku'] = $variants['sku'];
            $Product_var['select'] = [];
            unset($variants['price']);
            unset($variants['description']);
            unset($variants['featured_image']);
            unset($variants['sku']);
            foreach ($variants as $key_attr => $data_attr){
                $id = \App\Product::get_product_attributes_bylug($key_attr);
                $value_attr = [];
                $item = [];
                $attr = \App\Product::get_product_attributes_by_datatype(strtolower($data_attr),$id);
                if($attr) {
                    $Product_var['select'][] = $attr->id;
                }else{
                    $Product_var['select'][] = null;
                }
            }
                $product_variations[] = $Product_var;
        ...
        if ( isset($product_variations) ) {
            Product::update_meta_product($product_id, 'product_type', 1);
            Product::update_meta_product($product_id, 'product_variations', \GuzzleHttp\json_encode($product_variations));
        }
