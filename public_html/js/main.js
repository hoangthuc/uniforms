...
function payment_order() {
    var data = {billing:{},shipping:{},notes:''};
    var error = '';
    var same_bill = document.getElementById("same_as_billing").checked;
    var notes = document.getElementById("notes").value;
    data['notes'] = notes;
    data['same_bill'] = same_bill;
    $('#billing-form [name]').each(function () {
        var k = $(this).attr('name');
        var v = $(this).val();
        var r = $(this).attr('data-required');
        v = v.trim();
        data['billing'][k] = v;
        if (!v && r=='true') {
            $(this).addClass('error');
...
function get_data_reviews(page = 1) {
    ...
    $.ajax({
        ...
        data: {
            ...
            page: page,
            _token: setting.token,
            action: 'get_data_reviews_product'
        },
        success: function (resulf) {
            resulf = JSON.parse(resulf);
            setTimeout(function () {
...
function quick_view_product(event) {
    var product_id = $(event).attr('data-product');
    var display = document.querySelector('[display-quick-view-product]');
    if (display) {
...
/// display name plane single product
var $name_plate = {
    label:'',
    render:function(){
